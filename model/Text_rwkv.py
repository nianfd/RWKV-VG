...
class VL_RWKV(MyModule):
    def __init__(self, args):
        ...
        #self.emb = nn.Embedding(args.vocab_size, args.n_embd)
        self.pos_embedLarge = nn.Parameter(torch.zeros(420,1, 640))
        
        self.cls_token = nn.Parameter(torch.zeros(1, 1, 640))
    ...
    def forward(self, idx):
        ...
        B = idx.shape[1]
        # print(idx.shape)
        # print(self.pos_embedLarge.shape)
        x = idx + self.pos_embedLarge
        
        cls_tokens = self.cls_token.expand(-1, B, -1)
        #print(x.shape)
        #print(cls_tokens.shape)
        x = torch.cat((x, cls_tokens), dim=0)  # post cls_token
        
        #x = torch.cat((cls_tokens, x), dim=0)  # post cls_token
