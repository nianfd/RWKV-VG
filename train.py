...
def get_args_parser():
    ...
    parser.add_argument('--vl_hidden_dim', default=640, type=int,
    ...
    parser.add_argument('--detr_model', default='', type=str, help='detr model')
...
def main(args, argsRWKV):
    ...
    model = build_model(args, argsRWKV)
    #print('test0-0')
    model.to(device)
    #print('test0-1')
    ...
    tra_param = [p for n, p in model_without_ddp.named_parameters() if (("vl_rwkvmodel" in n) and p.requires_grad)]
    rest_param = [p for n, p in model_without_ddp.named_parameters() if (("vl_rwkvmodel" not in n) and p.requires_grad)]

    param_list = [{"params": rest_param},
                   {"params": tra_param, "lr": args.lr_bert}
                   ]
    # visu_cnn_param = [p for n, p in model_without_ddp.named_parameters() if (("visumodel" in n) and ("backbone" in n) and p.requires_grad)]
    # visu_tra_param = [p for n, p in model_without_ddp.named_parameters() if (("visumodel" in n) and ("backbone" not in n) and p.requires_grad)]
    ...
    # rest_param = [p for n, p in model_without_ddp.named_parameters() if (("visumodel" not in n) and ("textmodel" not in n) and p.requires_grad)]
    # #valid_param = [p for n, p in model_without_ddp.named_parameters() if (("vl_rwkvmodel" not in n)  and p.requires_grad)]
    # #print(text_tra_param)
    # # for n, p in model_without_ddp.named_parameters():
    # #     #if (("vl_rwkvmodel" not in n) and p.requires_grad):
    # #     print(n)
    # #valid_param = [p for n, p in model_without_ddp.named_parameters() if (("vl_rwkvmodel.image_model" not in n)  and p.requires_grad)]
    # valid_param = [p for n, p in model_without_ddp.named_parameters() if (("vl_rwkvmodel" not in n)  and p.requires_grad)]
    # #valid_param = [p for n, p in model_without_ddp.named_parameters() if (p.requires_grad)]
    # param_list = [{"params": valid_param},
...
if __name__ == '__main__':
    ...
    argsRWKV = load_model_configs('model_config/RWKV_CLIP_B32.json')
    assert argsRWKV.data_type in ["utf-8", "utf-16le", "numpy", "binidx", "dummy", "uint16"]
    assert argsRWKV.precision in ["fp32", "tf32", "fp16", "bf16"]
    assert argsRWKV.image_embed_dims == argsRWKV.n_embd
    if argsRWKV.text_initialization == "True":
        argsRWKV.text_initialization = True
    else:
        argsRWKV.text_initialization = False

    if argsRWKV.image_output_cls_token == "True":
        argsRWKV.image_output_cls_token = True
        argsRWKV.image_with_cls_token = True
    else:
        argsRWKV.image_output_cls_token = False
        argsRWKV.image_with_cls_token = False
        
    if argsRWKV.dim_att <= 0:
        argsRWKV.dim_att = argsRWKV.n_embd
    if argsRWKV.dim_ffn <= 0:
        argsRWKV.dim_ffn = int((argsRWKV.n_embd * 3.5) // 32 * 32) # default = 3.5x emb size
    if argsRWKV.text_num_head != 0:
        assert argsRWKV.n_embd % argsRWKV.text_num_head == 0
        argsRWKV.head_size = argsRWKV.n_embd // argsRWKV.text_num_head
    argsRWKV.with_cp = False
    
    os.environ["RWKV_CTXLEN"] = str(argsRWKV.ctx_len)
    os.environ["RWKV_HEAD_SIZE"] = str(argsRWKV.head_size)
    os.environ['RWKV_FLOAT_MODE'] = str(argsRWKV.precision)
    os.environ['Image_T_max'] = str((argsRWKV.input_size/argsRWKV.image_patch_size)**2)
    os.environ['Text_T_max'] = str(256)
    os.environ['Image_HEAD_SIE'] = str(argsRWKV.image_embed_dims // argsRWKV.image_num_heads)
    
    from model import Text_RWKV, Image_RWKV, get_model, tokenize, image_transform
    from model.utils import WarperCLIP_V_T_RWKV_text_change_head, WarperCLIP_V_T_RWKV_method, WarperCLIP_V_T_RWKV_text
    import datasets
